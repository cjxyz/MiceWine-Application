...
class TapGestureDetector(context: Context?,
     ...
     */
    private val mHandler: Handler = EventHandler(this)
    ...
    fun onTouchEvent(event: MotionEvent) {
        when (event.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                ...
                mHandler.sendEmptyMessageDelayed(0, ViewConfiguration.getLongPressTimeout().toLong())
                mPointerCount = 1
            }

            MotionEvent.ACTION_POINTER_DOWN -> {
                trackDownEvent(event)
                mPointerCount = max(mPointerCount.toDouble(), event.pointerCount.toDouble()).toInt()
            }
            ...
            MotionEvent.ACTION_UP -> {
                cancelLongTouchNotification()
                if (!mTapCancelled) mListener.onTap(mPointerCount, mInitialPoint!!.x, mInitialPoint!!.y)
                mInitialPoint = null
            }
    ...
    private class EventHandler(detector: TapGestureDetector) : Handler() {
        private val mDetector = WeakReference(detector)
